...
(function () {
  ...
  /** Date -> "2031-04" for <input type="month">. */
  ...
  function getLoanInputs() {
    ...
    return {
      ...
      oneTimeExtras: oneTimeExtras.map((o) => ({
        amount: Fmt.num(o.amount),
        date: parseMonthInput(o.date),
      })),
  ...
  function validateInputs(raw) {
    ...
    // One-time extra payments: clamp negative amounts to 0.
    inputs.oneTimeExtras = (inputs.oneTimeExtras || []).map((o) => ({
      amount: o.amount < 0 ? 0 : o.amount,
      date: o.date,
    }));
  ...
  function recompute() {
    ...
    const hasExtraPayments = inputs.extraMonthly > 0
      || inputs.oneTimeExtras.some((o) => o.amount > 0 && o.date)
  ...
  /* ================================================================
   * ONE-TIME EXTRA PAYMENTS (repeatable list)
   * Stored separately from the rest of the form since each row is
   * dynamically added/removed and has no fixed element id.
   * ================================================================ */
  const ONE_TIME_EXTRAS_KEY = 'mortgageCalc.oneTimeExtras.v1';

  let oneTimeExtras = [];

  function loadOneTimeExtras() {
    try {
      const raw = localStorage.getItem(ONE_TIME_EXTRAS_KEY);
  ...
  function saveOneTimeExtras() {
    try {
      localStorage.setItem(ONE_TIME_EXTRAS_KEY, JSON.stringify(oneTimeExtras));
    } catch (e) {
      // localStorage unavailable — ignore
    }
  }

  function renderOneTimeExtrasList() {
    UI.renderOneTimeExtras(oneTimeExtras);
  }
  ...
  function recomputeRefinance() {
    const includeExtras = $('refi-includeExtras').checked;
    let extras = null;

    if (includeExtras) {
      const { inputs } = validateInputs(getLoanInputs());
      extras = {
        startDate: defaultStartDate(), // "today" — extras apply going forward from now
        extraMonthly: inputs.extraMonthly,
        oneTimeExtras: inputs.oneTimeExtras,
        annualExtra: { amount: inputs.annualExtraAmount, month: inputs.annualExtraMonth },
        extraStartDate: inputs.extraStartDate,
  ...
  function init() {
    ...
    const restored = AppStorage.load();
    oneTimeExtras = loadOneTimeExtras();
    if (oneTimeExtras.length === 0) {
      oneTimeExtras.push({ amount: 0, date: '' });
    }
    renderOneTimeExtrasList();
    ...
    if (restored) {
      // Restoring values directly (without dispatching events) skips the
      // handlers below that keep dependent UI in sync — so do that by hand.
      $('pmiManualWrap').hidden = !$('pmiOverrideEnabled').checked;
      $('customTermWrap').hidden = $('loanTermYears').value !== 'custom';
      const checkedMethod = document.querySelector('input[name="closingCostMethod"]:checked');
      if (checkedMethod) {
        $('closingCostUnit').textContent = checkedMethod.value === 'dollar' ? '$' : '%';
      }
    }
    ...
    // ---- One-time extra payments: add / edit / remove rows ----
    ...
    $('oneTimeExtrasList').addEventListener('input', (e) => {
      const idx = Number(e.target.dataset.idx);
      if (Number.isNaN(idx) || !oneTimeExtras[idx]) return;
      if (e.target.dataset.field === 'amount') {
        oneTimeExtras[idx].amount = e.target.value;
      } else if (e.target.dataset.field === 'date') {
        oneTimeExtras[idx].date = e.target.value;
      }
      saveOneTimeExtras();
      // recompute() runs via the #loanForm 'input' listener above (this
      // listener fires first during bubbling, so state is up to date).
    });
    ...
    $('oneTimeExtrasList').addEventListener('click', (e) => {
      ...
      oneTimeExtras.splice(idx, 1);
      if (oneTimeExtras.length === 0) oneTimeExtras.push({ amount: 0, date: '' });
      saveOneTimeExtras();
      renderOneTimeExtrasList();
      recompute();
    });
    ...
    // ---- Persist every input change so values survive a page reload ----
    // (Runs after the form-specific handlers above, picking up final values.)
    document.addEventListener('input', () => AppStorage.save());
    document.addEventListener('change', () => AppStorage.save());

    // ---- Reset to defaults ----
    const resetBtn = $('resetDefaultsBtn');
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        if (!confirm('Reset all fields back to their default values? This clears your saved data on this device.')) return;
        AppStorage.clear();
