...
const UI = {
  ...
  renderRefinance(result) {
    ...
    document.getElementById('refi-currentInterestLabel').textContent = result.usingExtras
      ? 'Interest if you keep current loan (with extra payments)'
      : 'Interest if you keep current loan';
    document.getElementById('refi-newInterestLabel').textContent = result.usingExtras
      ? 'Interest with new loan (with extra payments)'
      : 'Interest with new loan';
  },
