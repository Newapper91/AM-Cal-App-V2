...
const UI = {
  ...
  /* ----------------------------------------------------------------
   * ONE-TIME EXTRA PAYMENTS LIST (repeatable rows)
   * ---------------------------------------------------------------- */
  renderOneTimeExtras(items) {
    const container = document.getElementById('oneTimeExtrasList');
    container.innerHTML = items.map((item, i) => `
      <div class="repeatable-row">
        <div class="input-prefix">
          <span>$</span>
          <input type="number" min="0" step="100" inputmode="decimal"
                 data-idx="${i}" data-field="amount" value="${item.amount}" />
        </div>
        <input type="month" data-idx="${i}" data-field="date" value="${item.date || ''}" />
        <button type="button" class="btn btn-remove btn-small" data-remove="${i}">Remove</button>
      </div>`).join('');
  },
  ...
  renderRefinance(result) {
    ...
    document.getElementById('refi-originalLifetime').textContent = Fmt.currencyRounded(result.originalLifetimeInterest);
    document.getElementById('refi-remainingCurrent').textContent = Fmt.currencyRounded(result.remainingInterestCurrent);
    document.getElementById('refi-alreadyPaid').textContent = Fmt.currencyRounded(result.interestAlreadyPaid);
    document.getElementById('refi-totalLifetimeRefi').textContent = Fmt.currencyRounded(result.totalLifetimeInterestWithRefi);

    const savingsEl = document.getElementById('refi-lifetimeSavings');
    if (result.lifetimeSavings >= 0) {
      savingsEl.textContent = Fmt.currencyRounded(result.lifetimeSavings) + ' saved';
      savingsEl.classList.remove('value-negative');
      savingsEl.classList.add('value-positive');
    } else {
      savingsEl.textContent = '−' + Fmt.currencyRounded(Math.abs(result.lifetimeSavings)) + ' (keeping current loan is cheaper)';
      savingsEl.classList.remove('value-positive');
      savingsEl.classList.add('value-negative');
    }

    document.getElementById('refi-extrasNote').hidden = !result.usingExtras;
    document.getElementById('refi-remainingCurrentLabel').textContent = result.usingExtras
      ? 'Remaining interest if you keep current loan (with extra payments)'
      : 'Remaining interest if you keep current loan';
    document.getElementById('refi-totalRefiLabel').textContent = result.usingExtras
      ? 'Lifetime interest with refinance (with extra payments)'
